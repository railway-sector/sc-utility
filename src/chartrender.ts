...
export function makeSeriesColumn({
  ...
  const statusColorMap: Record<string, any> = {
    ...
    comp: am5.color(seriesStatusColor[3] ?? "#0070ff"),
    exceeded: am5.color("#FF0000"),
    noactivity: am5.color("#000000"),
...
export function clickSeriesColumn({
  ...
  series.columns.template.events.on("click", (ev: any) => {
    ...
    where.chartCategory = categorySelected;
    where.chartCategoryType = typeof categorySelected;
    where.chartCategoryField = chartCategoryTypeField;
    where.status = statusArray.find(
      (item: any) => item.status === statusState,
    ).value;
    where.statusField = statusField;
...
export const highlightFilterLayerView = async ({
  ...
  const qe = where.queryExpression();
...
function convertFieldToLike(expression: any, targetField: any) {
  const regex = new RegExp(`${targetField}\\s*=\\s*'([^']*)'`, "g");
  return expression.replace(
    regex,
    (_match: any, value: any) => `${targetField} LIKE '%${value}%'`,
  );
}
...
export const sublayersQuery = ({
  ...
  const q = where.queryExpression();
  const qe = q.includes("DocName") ? convertFieldToLike(q, "DocName") : q;
...
class ChartStackColumnRender {
  ...
  constructor(options: chartColumnType & BuildingSublayerView) {
    this.revit = options.revit;
    ...
    this.chartCategoryTypes = options.chartCategoryTypes;
    this.chartCategoryTypeField = options.chartCategoryTypeField;
    this.statusTypename = options.statusTypename;
    this.statusStatename = options.statusStatename;
    this.statusArray = options.statusArray;
    this.statusField = options.statusField;
    this.seriesStatusColor = options.seriesStatusColor;
    this.strokeColor = options.strokeColor;
    this.strokeWidth = options.strokeWidth;
    this.view = options.view;
    this.setLayerViewFilter = options.setLayerViewFilter;
    this.new_chartIconSize = options.new_chartIconSize;
    this.new_axisFontSize = options.new_axisFontSize;
    this.chartIconPositionX = options.chartIconPositionX;
    this.chartPaddingRightIconLabel = options.chartPaddingRightIconLabel;
