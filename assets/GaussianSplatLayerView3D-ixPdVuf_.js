...
return -0.5 * dot(conic, vec3(x * x, 2.0 * x * y, y * y));
    ...
    uvec4 fetchPackedGaussian(uint gaussianIndex) {
      ...
      uint gaussianIndexY = gaussianIndex >> ${Jt}u;
    ...
    uvec4 fetchPackedGaussianHeader(uint gaussianIndex) {
      ...
      uint headerIndexY = headerIndex >> ${Jt}u;
  ...
  `)}const Zt=""+(se-1),Jt=`${Math.log2(se)}`,ka=""+(ye-1),La=`${Math.log2(ye)}`;let mi=class extends Tt{constructor(){super(...arguments),this.tileCameraPosition=q(),this.cameraDelta=q()}};function gi(n){n.code.add(x`float unpackOpacity(uvec4 packedGaussian) {
